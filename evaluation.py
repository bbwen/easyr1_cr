...
def main(global_args,local_configs):
    ...
    for config in local_configs:
        ...
        # prompt_ids = tokenizer.apply_chat_template(to_tokenize,add_generation_prompt=True,tokenize=False)  # explicit
        # texts = [tokenizer.decode(x) for x in prompt_ids]

        prompt_ids = tokenizer.apply_chat_template(to_tokenize, add_generation_prompt=True, tokenize=False)
        texts = prompt_ids  # Already strings when tokenize=False   ###########################################################
        ...
        llm = LLM(model=config.model, gpu_memory_utilization=global_args.gpu_memory_utilization, trust_remote_code=True, dtype="bfloat16", enforce_eager=True, max_model_len=4096)
